...
-- FASE: Día 6 - Construcción BD Relacional (Schema Consolidado y Corregido)
-- VERSIÓN FINAL: incluye las correcciones acordadas en la revisión conjunta.
--
-- Cambio aplicado en esta versión respecto al archivo anterior:
--   - contrato.tipo_contrato  : VARCHAR(50) -> ENUM
--   - nomina.estado_pago      : VARCHAR(20) -> ENUM
--   (única observación que seguía pendiente; el resto ya estaba corregido)
-- ==============================================================================

CREATE DATABASE IF NOT EXISTS rrhh_itn;
USE rrhh_itn;

-- ------------------------------------------------------------------------------
-- 1. TABLAS MAESTRAS (Nivel 0 - Sin dependencias externas)
-- ------------------------------------------------------------------------------
CREATE TABLE sede (
...
CREATE TABLE departamento (
    ...
    id_gerente INT NULL, -- Se actualiza luego vía ALTER TABLE
...
CREATE TABLE puesto (
    ...
    salario DECIMAL(10, 2) NOT NULL COMMENT 'Salario base/rango sugerido para el puesto',
...
-- ------------------------------------------------------------------------------
-- 2. TABLAS TRANSACCIONALES (Nivel 1 - Dependen de Catálogos)
-- ------------------------------------------------------------------------------
CREATE TABLE empleado (
    ...
    id_puesto INT NOT NULL, -- La sede y el departamento se obtienen a través del puesto (3FN)
    salario_actual DECIMAL(10,2) NOT NULL COMMENT 'Salario vigente del empleado (puede diferir del salario del puesto)',
...
-- Resolución de dependencia circular: Asignar gerente al departamento
...
-- ------------------------------------------------------------------------------
-- 3. TABLAS DE SISTEMA Y AUDITORÍA (Nivel 2)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 4. TABLAS OPERATIVAS (Nivel 2 - Dependen del Empleado)
-- ------------------------------------------------------------------------------
CREATE TABLE contrato (
    id INT AUTO_INCREMENT PRIMARY KEY,
    id_empleado INT NOT NULL,
    id_puesto INT NOT NULL,
    -- CORREGIDO: antes era VARCHAR(50), ahora ENUM para consistencia con el resto
    -- de campos categóricos del modelo (empleado.estado, contrato.estado, etc.)
    tipo_contrato ENUM('INDEFINIDO','TEMPORAL','POR_OBRA','PASANTIA') NOT NULL,
    fecha_inicio DATE NOT NULL,
    fecha_fin DATE NULL,
    salario_pactado DECIMAL(10,2) NOT NULL COMMENT 'Salario acordado en este contrato específico',
    estado ENUM('ACTIVO','INACTIVO') NOT NULL DEFAULT 'ACTIVO',
    ...
    CONSTRAINT fk_contrato_empleado FOREIGN KEY (id_empleado) REFERENCES empleado(id) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE asistencia (
    ...
    id_sede INT NOT NULL COMMENT 'Sede donde se marcó (puede diferir de la sede base del empleado)',
...
CREATE TABLE nomina (
    ...
    -- CORREGIDO: antes era VARCHAR(20), ahora ENUM para consistencia
